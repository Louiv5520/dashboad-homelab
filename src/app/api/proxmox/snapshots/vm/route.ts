...
export async function GET(request: Request) {
  ...
  try {
    ...
    const snapshots = await listSnapshots("qemu", vmid);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const task = await createSnapshot({ ...input, source: "qemu" });
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const task =
      input.mode === "rollback"
        ? await rollbackSnapshot({ vmid: input.vmid, snapname: input.snapname, source: "qemu" })
        : await deleteSnapshot({ vmid: input.vmid, snapname: input.snapname, source: "qemu" });
    return Response.json({ ok: true, task });
  } catch (error) {
